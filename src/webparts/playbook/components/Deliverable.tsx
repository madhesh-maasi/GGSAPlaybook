...
const Deliverable = (props) => {
  ...
  useEffect(() => {
    ...
    arrDlble = arrDlble.map((li) => ({ Title: li.title, Details: li.details, isShow: false }));
  ...
  return (
    ...
    >
      {currDeliver.length > 0 ? currDeliver.map((item) => {
        return <div>
          {item.isShow == true &&
            <div
              style={{
              ...
              onClick={() => {
                arrDlble.filter((fiItem) => fiItem.Title == item.Title)[0].isShow = false
                setCurrDeliver([...arrDlble]);
              }}
            >
              {item.Title == "Planning"
                ? <>
                  ...
                  />
                  {item.Details.length > 0 ? item.Details.map((row) => {
                    return <div
                      className={style.deliverOpen}
                    >
                      <li>{row}</li>
                    </div>
                  }) :
          ...
          {item.isShow == false &&
            <div
              onClick={() => {
                ...
                arrDlble.filter((fiItem) => fiItem.Title == item.Title)[0].isShow = true
                setCurrDeliver([...arrDlble]);
              }}
              style={{
                color: "#f99d26",
            ...
            >
              <div className={style.cardImg}>
                {item.Title == "Planning" 
                  ? <img src={`${planningImg}`} alt="planning" width="30px" />
                  : item.Title == "Development"
                  ? <img src={`${devImg}`} alt="planning" width="30px" />
                  : item.Title == "Product/solution"
                  && <img src={`${productImg}`} alt="planning" width="30px" />
                }
              </div>
              {item.Title}
            </div>
